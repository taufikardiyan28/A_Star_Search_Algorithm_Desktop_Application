class AStar:
    ...
    def get_path(self, start, end, OBSTACLES, SIZE, diagonal_state):
        ...
        while openList:
            ...
            for tmp in openList[1:]:
                if nodes[tmp][1] + self.get_distance(tmp, end) < nodes[current_node][1] + self.get_distance(current_node, end):
                    current_node = tmp
            x, y = current_node
            if current_node == end:
                break
